...
template <typename F> auto timeit(F func, size_t bench_size) -> vector<double> {
...
void show_timeit(const vector<double> &result, std::ofstream &log_file) {
  ...
  double stddev =
      std::sqrt(std::accumulate(result.begin(), result.end(), 0.0,
  ...
  log_file << std::format(
      "mean: {:.4f}, stddev: {:.4f}, min: {:.4f}, max: {:.4f}\n", mean, stddev,
      min, max);
  log_file << "\n";
}
