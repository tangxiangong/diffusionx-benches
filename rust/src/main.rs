...
fn show_timeit(result: Vec<f64>, log_file: &mut File) {
    ...
    let stddev = (result.iter().map(|x| (x - mean).powi(2)).sum::<f64>() / result.len() as f64).sqrt();
    let min = *result
        .iter()
    ...
    writeln!(log_file, "mean: {mean:.4}, stddev: {stddev:.4}, min: {min:.4}, max: {max:.4}").expect("Failed to write to log file");
...
fn main() {
    ...
    writeln!(log_file, "=========================Rust==========================").expect("Failed to write to log file");
    writeln!(log_file).expect("Failed to write to log file");

    writeln!(log_file, "bench size: {}, length of random vectors: {}", bench_size, len).expect("Failed to write to log file");
    ...
    writeln!(log_file, "------------uniform random number sampling------------").expect("Failed to write to log file");
    show_timeit(timeit(uniform, bench_size), &mut log_file);

    let normal = || {
        let _rands = normal::standard_rands(len);
        0
    };
    writeln!(log_file, "------------normal random number sampling------------").expect("Failed to write to log file");
    ...
    writeln!(log_file, "------------stable random number sampling------------").expect("Failed to write to log file");
